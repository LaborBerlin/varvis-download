...
describe("rangedUtils", () => {
  ...
  describe("rangedDownloadVCF", () => {
    ...
    test("should handle tabix command failure", async () => {
      ...
      await expect(rangedDownloadVCF(url, range, outputFile, mockLogger, false))
  ...
  describe("indexVCF", () => {
    ...
    test("should call spawn with correct tabix index arguments", async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(`Indexing VCF.gz file: ${vcfGzFile}`);
      expect(mockLogger.info).toHaveBeenCalledWith(`Indexed VCF.gz file: ${vcfGzFile}`);
    });
    ...
    test("should handle indexing failure", async () => {
      ...
      await expect(indexVCF(vcfGzFile, mockLogger, false))
        .rejects.toThrow("Process tabix exited with code 1");

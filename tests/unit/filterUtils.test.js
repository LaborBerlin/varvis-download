const { applyFilters, parseFilterExpression, applyFilter } = require('../../js/filterUtils');
...
describe('filterUtils', () => {
  ...
  describe('parseFilterExpression', () => {
    test('should parse equality filter expression correctly', () => {
      ...
      expect(filter).toEqual({ field: 'analysisType', operator: '=', value: 'SNV' });
    });

    test('should parse inequality filter expression correctly', () => {
      const filter = parseFilterExpression('analysisType!=CNV');
      expect(filter).toEqual({ field: 'analysisType', operator: '!=', value: 'CNV' });
    });
    ...
    test('should trim whitespace from value', () => {
      ...
      expect(filter).toEqual({ field: 'sampleId', operator: '=', value: 'sample1' });
    });

    test('should throw error for invalid filter expression', () => {
      expect(() => parseFilterExpression('invalid')).toThrow('Invalid filter expression: invalid');
      expect(() => parseFilterExpression('field=')).toThrow('Invalid filter expression: field=');
    });
  });
  ...
  describe('applyFilter', () => {
    ...
    test('should throw error for unsupported operator', () => {
      const filter = { field: 'score', operator: '>=', value: '15' };
      expect(() => applyFilter(analyses, filter)).toThrow('Unsupported operator: >=');
    });
  });
  ...
  describe('applyFilters', () => {
    ...
    test('should handle complex filtering scenarios', () => {
      const filtered = applyFilters(analyses, ['score<25', 'analysisType!=CNV']);
      expect(filtered).toHaveLength(2);
      expect(filtered.map(a => a.id)).toEqual([1, 3]);
    });
  });

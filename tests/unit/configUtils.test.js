...
describe('configUtils', () => {
  ...
  describe('loadLogo', () => {
    test('should load logo from file when it exists', () => {
      ...
      expect(fs.existsSync).toHaveBeenCalledWith(expect.stringContaining('assets/logo.txt'));
      expect(fs.readFileSync).toHaveBeenCalledWith(expect.stringContaining('assets/logo.txt'), 'utf-8');
  ...
  describe('getLastModifiedDate', () => {
    ...
    test('should throw error if file does not exist', () => {
      ...
      expect(() => getLastModifiedDate('/path/to/nonexistent.txt')).toThrow('ENOENT');
    });

...
https://mytarget.varvis.com/download/analysis/12345/sample_001.vcf.gz.tbi?token=abc123
```
...
# Only index files
./varvis-download.cjs -t mytarget -a 12345 -f "bam.bai,vcf.gz.tbi"
```
...
```bash
./varvis-download.cjs -t mytarget -s "LIMS-001,LIMS-002"
```
...
```bash
./varvis-download.cjs -t mytarget -l "LIMS_123,LIMS_456"
```
...
```bash
./varvis-download.cjs -t mytarget -s "LIMS-001" -l "LIMS_123" -a "12345"
```
...
```bash
./varvis-download.cjs -t mytarget -a 12345 --logfile "./logs/download_$(date +%Y%m%d).log"
```
...
```bash
./varvis-download.cjs -t mytarget -a 12345 --reportfile "./reports/analysis_12345_report.json"
```
...
```bash
./varvis-download.cjs -t mytarget -a 12345 --loglevel debug --logfile debug.log
```
...
./varvis-download.cjs -t mytarget -a "12345,67890" -d "./mytarget/"

# Download from secondary target
./varvis-download.cjs -t othertarget -a "11111,22222" -d "./othertarget/"
```
...
for ANALYSIS in "${ANALYSES[@]}"; do
    ...
    ./varvis-download.cjs -t mytarget -a "$ANALYSIS" -d "./data/analysis_$ANALYSIS"
done
```
...
while IFS= read -r SAMPLE; do
    ...
    ./varvis-download.cjs -t mytarget -s "$SAMPLE" -d "./data/$SAMPLE/"
done < samples.txt
```
...
./varvis-download.cjs -t mytarget -a 12345 --list | grep -E "GB|TB"
```
...
./varvis-download.cjs -t mytarget -a 12345 -g "chr1:155000000-156000000" -f "vcf.gz,vcf.gz.tbi"
# Creates: sample.chr1_155000000_156000000.vcf.gz

# Extract multiple regions (creates separate files)
./varvis-download.cjs -t mytarget -a 12345 -g "chr1:155000000-156000000 chr17:41000000-42000000" -f "vcf.gz,vcf.gz.tbi"
...
./varvis-download.cjs -t mytarget -a 12345 -b regions.bed -f "vcf.gz,vcf.gz.tbi"
```
...
./varvis-download.cjs -t mytarget -a 12345 -g "chr1:155000000-156000000" -f "bam,bam.bai"

# Multiple regions with BED file
./varvis-download.cjs -t mytarget -a 12345 -b target_regions.bed -f "bam,bam.bai"
```
...
# List expected files
./varvis-download.cjs -t mytarget -a "$ANALYSIS" --list > expected_files.txt
